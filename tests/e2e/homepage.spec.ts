...
test("homepage presents the local workspace clearly", async ({ page }) => {
  ...
  await expect(page.getByRole("link", { name: "Star Markdown Lens on GitHub" }).first()).toHaveAttribute("href", "https://github.com/ayushhagarwal/markdown-lens");
  await expect(page.getByRole("contentinfo").getByRole("link", { name: "Star Markdown Lens on GitHub" })).toHaveClass(/focus-visible:ring-2/);
